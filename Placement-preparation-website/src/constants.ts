...
export const MOCK_QUESTIONS: Record<string, any[]> = {
  ...
  "Data Structures & Algorithms": [
    { id: 101, text: "Which data structure is based on LIFO principle?", options: ["Queue", "Stack", "Tree", "Graph"], correct: 1, level: "Easy", explanation: "Stack uses Last In First Out." },
    { id: 102, text: "What is the time complexity of binary search in a sorted array?", options: ["O(n)", "O(1)", "O(log n)", "O(n^2)"], correct: 2, level: "Easy", explanation: "Binary search halves the search space iteratively." },
    { id: 103, text: "Which of the following sorting algorithms is considered the fastest on average?", options: ["Bubble Sort", "Quick Sort", "Selection Sort", "Insertion Sort"], correct: 1, level: "Medium", explanation: "Quick Sort has an average case of O(n log n) with low constant factors." },
    { id: 104, text: "What data structure is used for Breadth First Search of a graph?", options: ["Stack", "Queue", "Priority Queue", "Heap"], correct: 1, level: "Medium", explanation: "Queue allows processing nodes level by level." },
    { id: 105, text: "A balanced binary tree with 'N' nodes has a height of:", options: ["O(n)", "O(n log n)", "O(1)", "O(log n)"], correct: 3, level: "Easy", explanation: "A balanced tree maintains a height logarithmic to the number of nodes." }
  ],
  "Core CS": [
    { id: 201, text: "Which OSI layer is responsible for routing?", options: ["Data Link", "Network", "Transport", "Physical"], correct: 1, level: "Easy", explanation: "Network layer handles IP addressing and routing." },
    { id: 202, text: "What is a major advantage of indexing in databases?", options: ["Reduces storage", "Speeds up read queries", "Speeds up write queries", "Encrypts data"], correct: 1, level: "Medium", explanation: "Indexes allow O(log n) or O(1) lookups instead of full table scans." },
    { id: 203, text: "Which HTTP method is idempotent?", options: ["POST", "PATCH", "PUT", "CONNECT"], correct: 2, level: "Medium", explanation: "PUT is idempotent, meaning repeated identical requests have the same effect as a single request." },
    { id: 204, text: "In OS, what prevents processes from accessing each other's memory?", options: ["Virtual Memory", "Cache", "Thrashing", "Paging"], correct: 0, level: "Hard", explanation: "Virtual memory maps logical addresses to physical memory, isolating processes." },
    { id: 205, text: "What does ACID stand for in DBMS?", options: ["Atomicity, Consistency, Isolation, Durability", "Atomicity, Concurrency, Isolation, Durability", "Accuracy, Consistency, Independence, Durability", "Atomicity, Consistency, Independence, Database"], correct: 0, level: "Medium", explanation: "ACID properties ensure reliable database transactions." }
  ]
};

export const PLACEMENT_MEGA_BANK = [
  ...MOCK_QUESTIONS["Quantitative Aptitude"].slice(0, 5).map(q => ({...q, section: "Aptitude"})),
  ...MOCK_QUESTIONS["Logical Reasoning"].slice(0, 5).map(q => ({...q, section: "Reasoning"})),
  ...MOCK_QUESTIONS["Verbal Ability"].slice(0, 5).map(q => ({...q, section: "Verbal & Writing"})),
  ...MOCK_QUESTIONS["Data Structures & Algorithms"].slice(0, 5).map(q => ({...q, section: "DSA"})),
  ...MOCK_QUESTIONS["Core CS"].slice(0, 5).map(q => ({...q, section: "Technical"}))
];

export const ROLE_REQUIREMENTS: Record<string, { sections: string[], threshold: number }> = {
  "Software Engineer": { sections: ["DSA", "Technical", "Reasoning"], threshold: 60 },
  "Frontend Developer": { sections: ["DSA", "Reasoning", "Verbal & Writing"], threshold: 50 },
  "Backend Developer": { sections: ["DSA", "Technical"], threshold: 70 },
  "Data Scientist": { sections: ["Aptitude", "Reasoning", "DSA"], threshold: 70 },
  "UI/UX Designer": { sections: ["Verbal & Writing", "Reasoning"], threshold: 60 },
  "Product Manager": { sections: ["Verbal & Writing", "Aptitude", "Reasoning", "Technical"], threshold: 60 },
  "Full Stack Developer": { sections: ["DSA", "Technical"], threshold: 65 }
};
